...
public class ArgsParser{
	...
	int setArgs(String[] args, Properties argsAsProps){
		...
		for(int i = 0; i < args.length; i++){
			...
			if(arg.startsWith("-")){//treat as an option
				...
				if(arg.startsWith("-D")){ //if we'll want it in java way (no, we want POSIX!!!)
				...
				else if (arg.equals("-s") || arg.equals("--snips")){
					i++;
					if(i>=args.length){
						System.out.println("Option "+args[i-1]+" requires argument");
						System.out.println();
					...
					argsAsProps.put(Constants.SNIPS_MAPPING_KEY, args[i]);
				}
					...
					argsAsProps.put(Constants.SM_THRESHOLD_KEY, args[i]);
				}
					...
					argsAsProps.put(Constants.MIN_THRESHOLD_KEY, args[i]);
				}			
				else if (arg.equals("--cg")){
					argsAsProps.put(Constants.SHOW_SIMILARITY, "true");
					argsAsProps.put(Constants.SIMILARITY_METRICS_1, "true");
				}
				else if (arg.equals("--cg2")){
					argsAsProps.put(Constants.SHOW_SIMILARITY, "true");
					argsAsProps.put(Constants.SIMILARITY_METRICS_2, "true");
				}
				else if (arg.equals("--cg3")){
					argsAsProps.put(Constants.SHOW_SIMILARITY, "true");
					argsAsProps.put(Constants.SIMILARITY_METRICS_3, "true");
				}		
				else if (arg.equals("--hapmap")){
					argsAsProps.put(Constants.HAPMAP_KEY, "true");
				}
				else if (arg.equals("--phase")){
					argsAsProps.put(Constants.PHASE_KEY, "true");
				}					
				else if (arg.equals("-u") || arg.equals("--hir")){
					argsAsProps.put(Constants.UPS_MODE_KEY, "true");
				}
				else if (arg.equals("-b")){
					argsAsProps.put(Constants.BEHAR_MODE_KEY, "true");
				}
				else if (arg.equals("-p") || arg.equals("--ped2raw")){
					argsAsProps.put(Constants.PED2RAW_MODE_KEY, "true");
				}				
				else if (arg.equals("-z") || arg.equals("--roh")){
					argsAsProps.put(Constants.HOMOZYGOUSITY, "true");
				}
				else if (arg.equals("-nc")){
					argsAsProps.put(Constants.TREAT_NO_CALLS_AS_HOMOZYGOUS, "false");
				}
				else if (arg.equals("-M") || arg.equals("--merge")){
					argsAsProps.put(Constants.MERGE_MODE_KEY, "true");
				}
	...
	private void printUsage(){
	   ...
	   String progName="aisconvert"; 
	   
	   System.out.println("");
	   System.out.println(" " + progName+" - Genome Processing Tool "+Constants.releaseVersionString);
	   System.out.println(" Copyright (C) 2009 Vasili Gavrilov");
	   ...
	   System.out.println("     "+progName+" -i 1.txt --snips data/snips.txt");
	   System.out.println("                     Passing required snips (If snips are not passed - there");
	   System.out.println("                     will be a warning and default snips file will be used");                       
	   
	   System.out.println("     "+progName+" -u -i 1.txt -i 2.txt");
	   System.out.println("                     HIR calculator: compare 2 files");

	   System.out.println("     "+progName+" -u -i /home/data");
	   System.out.println("                     HIR calculator: compare each file with every other file");

	   System.out.println("     "+progName+" -u -i /home/data -m 500 -cM 5");
	   System.out.println("                     HIR calculator: passing snips and centimorgans thresholds");
	   
	   System.out.println("     "+progName+" -u -i /home/data -m 500 -cM 5 --cg");
	   System.out.println("                     Additionally show similarity percentage - for the 2 genomes");
	   	
	   System.out.println("     C:\\aaa\\aisconvert>aisconvert -p data\\test_short.ped");
	   System.out.println("                     Convert PED format for a test file to RAW format using test mapping");
	   
	   System.out.println("     "+progName+" -p -i C:\\data\\VVerenich.ped -s C:\\data\\VVerenich.map -d C:\\data\\results_dir");
	   System.out.println("                     Convert PED format to RAW format (23andme file layout) using map provided");
	   ...
	   System.out.println(" -s, --snips        File path of the list of snips - how output will be ordered");
